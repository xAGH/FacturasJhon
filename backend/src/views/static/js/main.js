...
function desativateButtons(parent){
    ...
    document.getElementById('removeItemButton').style.visibility = "hidden";
    document.getElementById('generateInvoiceButton').style.visibility = "hidden";
}
...
window.onload = () => {
    ...
    let el = elements[i].href.split('/').slice(3);
    let route = ""
    for (let j = 0; j < el.length; j++) {
        route += "/" + el[j];
    }
    if (route == window.location.pathname) {
        elements[i].classList.add('md:text-blue-700');
    }
    }
}
</script>